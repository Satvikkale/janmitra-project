...
export default function NGOUsers() {
  ...
  // Paid Organization booking state
  const [paidOrganizations, setPaidOrganizations] = useState<PaidOrganization[]>([]);
  const [loadingOrganizations, setLoadingOrganizations] = useState(false);
  const [showBookingModal, setShowBookingModal] = useState(false);
  const [selectedOrg, setSelectedOrg] = useState<PaidOrganization | null>(null);
  const [selectedService, setSelectedService] = useState<string>('');
  const [bookingForm, setBookingForm] = useState<ServiceBooking>({
    organizationId: '',
    organizationName: '',
    service: '',
    name: '',
  ...
  const fetchPaidOrganizations = async () => {
    try {
      ...
      setPaidOrganizations(orgsData.map((org: any) => ({
        ...
        description: org.description || org.businessType || '',
      })) || []);
    } catch (err) {
      console.error('Error fetching organizations:', err);
      // Use empty array if fetch fails
      setPaidOrganizations([]);
    } finally {
  ...
  const openBookingModal = (org: PaidOrganization) => {
    setSelectedOrg(org);
    setSelectedService('');
    setBookingForm({
  ...
  const handleServiceSelect = (service: string) => {
    ...
    setBookingForm(prev => ({ ...prev, service }));
  };

  const handleBookingFormChange = (field: keyof ServiceBooking, value: string) => {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {userData && (
          <div className="space-y-6">
            ...
            {/* Paid Organizations Section */}
            <div className="bg-white rounded-lg p-6 shadow-sm">
              <h3 className="text-xl font-bold text-gray-900 mb-1">Quick Actions - Paid Services</h3>
              <p className="text-gray-600 text-sm mb-6">Book professional services from verified organizations</p>
              ...
              {loadingOrganizations ? (
                ...
                <div className="bg-gray-100 p-8 rounded-lg text-center">
                  <p className="text-gray-500">No paid organizations available at the moment</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {paidOrganizations.map((org) => (
                    <div key={org.id} className="border border-gray-200 rounded-lg p-5 hover:shadow-lg transition-shadow">
                      <div className="mb-3">
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-lg shadow-lg max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">{selectedOrg.name}</p>
                </div>
                <button
                  onClick={() => {
                    setShowBookingModal(false);
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {selectedOrg.services && selectedOrg.services.map((service, idx) => (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe your requirements, location, preferred date/time, etc."
                    rows={4}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                  />
                </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={submittingBooking || !selectedService}
